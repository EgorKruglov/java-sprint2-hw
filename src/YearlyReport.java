...
public class YearlyReport {
    boolean isReport = false;   // Флаг о проверке отчёта
    ...
    void getYearInfo() {    // Вывод информации о годовом отчёте
        ...
        for (int i = 0; i < yearStat.size(); i++) {    // Поиск среднего дохода и расхода за год
            if (yearStat.get(i).isExpense) {           // Найдём все траты
                expensesSum += yearStat.get(i).amount;
            } else {                                    // Найдём все доходы
                profitSum += yearStat.get(i).amount;
            }
        }
